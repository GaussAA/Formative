...
flowchart TB
  ...
  %% Risk points
  N2 -.-> W1["⚠️ Repetition Loop<br/>ask_plan not bound to answers"]
  N0 -.-> W2["⚠️ Wrong Domain<br/>use ecommerce blueprint for community"]
  N1 -.-> W3["⚠️ Field Drift<br/>non-canonical values / schema mismatch"]
  W1 -.-> Obs
  W2 -.-> Obs
  W3 -.-> Obs
```
...
sequenceDiagram
  ...
  Note over Planner: ⚠️ 常见故障点：<br/>1) domain未切换→蓝图错<br/>2) answered字段没确认→重复问<br/>3) canonical不一致→字段漂移
```
